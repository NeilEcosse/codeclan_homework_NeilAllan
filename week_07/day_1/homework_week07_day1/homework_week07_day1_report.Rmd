...
The first figure to note is that more than three times as many trips were made by males than females, which feels like it could be a cause for concern.
...
If we repeat the analysis on usage over the year and by time of day but split the results by gender, we see a broadly similar outcome.
...
The lowest number of trips recorded at a start station was 1, and the highest 434.
...
# Create colour scheme for marker based on number of trips
start_locations <- start_locations %>% 
...
# Key points and suggested further analysis
